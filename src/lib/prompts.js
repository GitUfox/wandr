...
const DETAILS_RULE = `DETAILS FORMAT: each Details cell is one factual sentence, then fact tokens separated by " · " (space, middle dot, space) in this exact order: cost, duration, opening-hours note, booking note. The sentence says what the place is and why it's worth the time, in 20 words or fewer — it must NOT contain any fact a token carries: no prices, no durations, no opening hours, no booking language in the sentence, ever. Omit any token that doesn't apply — never pad, and never write negative or filler tokens ("no booking needed", "accessible all hours"): omit those entirely. "free" alone is a valid cost token. Keep tokens terse ("~€15", "2h", "opens 09:00", "book ahead"). NEVER use the "|" character inside a cell. Example cell: Moorish hilltop citadel with the best panorama over Alfama. · ~€15 · 2h · opens 09:00 · book ahead`;
